...
#To get the FHIR Procedure_resource
    def make_procedures_from_log(self):
        df = pd.read_csv(self.procedures_filename)
        d={'procedure':[]}
...
#To read medication_adminstrations_note
    ...
    def read_medication_administrations_filename(self):
        with open(self.medication_administrations_filename,"r+") as f:
            d=json.load(f)   
        return d
    def calculate_vis_timeseries(self):
        ds=VISCalculator.list_vis_timeseries(self)
        df = pd.DataFrame(ds) 
        df.columns =['VIS', 'effectivePeriod_start', 'effectivePeriod_end' ]
        df = df.reindex(columns=['effectivePeriod_start', 'effectivePeriod_end','VIS'])
        df=df.drop('effectivePeriod_end',axis=1)

        
        return df   

    def list_vis_timeseries(self):
